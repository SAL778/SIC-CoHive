...
function Statistics() {
			...
			onSuccess: (miscStatData) => {
				...
				setCardData([
					...
					},
					{
						...
						value:
							miscStatData.TotalRoomBookings +
							miscStatData.TotalEquipmentBookings,
					},
					{
						icon: "fa fa-clock",
						label: "Average Booking Time",
						value: `${Math.round(
							miscStatData.averagEbookingDurationHour
						)} minutes`,
					},
				]);
			},
	...
	const commonTimePickerProps = () => ({
		...
		placeholder:
			selectedTimeScope.value == "all" ? "Showing all time" : "Pick a date",
		//Default to no value if either isn't supplied
		//Month zero indexed
		value:
			!!selectedYear && !!selectedMonth && selectedTimeScope.value !== "all"
				? new Date(selectedYear + 1900, selectedMonth, 1)
				: null,
	});
	...
	return (
		<div className="w-full">
			<section className="heading flex min-h-0 gap-auto my-8 justify-between">
				<div>
					<h1 className="text-orange-600 text-5xl font-bold capitalize">
						{selectedTimeScope.label}
					</h1>
					<h2 className="text-neutral-400 text-2xl font-medium capitalize">
						Statistics
					</h2>
				</div>
...
function StatCard({ title, content, icon = null, className }) {
	...
	return (
		<div className={`${className} relative overflow-hidden shadow-custom p-6`}>
			<h2 className="text-xl text-neutral-600 font-regular z-10 mb-6">
				{title}
			</h2>
			<p className="text-5xl text-orange-600 font-extrabold z-10">{content}</p>
			{icon && (
				<i
					className={`${icon} text-[#ededed] text-[128px] absolute -translate-x-12`}
				/>
			)}
		</div>
	);
